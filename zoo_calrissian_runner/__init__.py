...
class ZooCalrissianRunner(BaseRunner):
    ...
    def execute(self, wall_time=None):
        ...
        # Stage out CWL File outputs that weren't handled by the CWL stage-out step
        if self.execution.is_succeeded() and output:
            try:
                additional_params = self.handler.get_additional_parameters()
                output = self._stage_out_file_outputs(output, additional_params)
            except Exception as e:
                logger.error(f"Failed to stage file outputs: {e}")
                logger.exception(e)

        # Normalize output values to {"value": URI} format for post_execution_hook
        if output:
            for oname, ovalue in list(output.items()):
                if isinstance(ovalue, str):
                    output[oname] = {"value": ovalue}
                elif isinstance(ovalue, dict) and "value" not in ovalue:
                    if ovalue.get("class") == "File":
                        logger.warning(f"Output '{oname}' is a raw CWL File (stage-out may have failed), skipping normalization")
                    else:
                        logger.warning(f"Output '{oname}' has no 'value' key: {list(ovalue.keys())}")
    ...
    def _stage_out_file_outputs(self, output: dict, additional_params: dict) -> dict:
        """Stage out CWL File outputs to S3 and create STAC catalogs.
        
        Called when eoap_cwlwrap doesn't generate stage-out steps for File-type outputs.
        Copies files from the calrissian volume, uploads to S3, creates STAC catalogs,
        and updates the output dict with {"value": "http://...catalog.json"}.
        """
        import boto3

        aws_access_key_id = additional_params.get("aws_access_key_id", "")
        aws_secret_access_key = additional_params.get("aws_secret_access_key", "")
        endpoint_url = additional_params.get("endpoint_url", "").rstrip("/")
        region_name = additional_params.get("region_name", "us-east-1")
        s3_bucket = additional_params.get("s3_bucket", "results")
        sub_path = additional_params.get("sub_path", "default")
        ...
        for output_name, output_value in output.items():
            if not isinstance(output_value, dict):
                continue
            if output_value.get("class") != "File":
                continue
            basename = output_value.get("basename", "")
            if not basename:
                continue

            logger.info(f"Staging out File output '{output_name}': {basename}")
            try:
                local_files = self.execution.get_file_from_volume([basename])
                local_file = local_files[0]

                if not os.path.exists(local_file):
                    logger.error(f"File not found after copy: {local_file}")
                    continue
                ...
                s3_file_key = f"{sub_path}/{output_name}/{basename}"
                s3_client.upload_file(local_file, s3_bucket, s3_file_key)
                file_http_uri = f"{endpoint_url}/{s3_bucket}/{s3_file_key}"
                logger.info(f"Uploaded {basename} to s3://{s3_bucket}/{s3_file_key}")

                now_str = datetime.now(tz=timezone.utc).isoformat()
                base_url = f"{endpoint_url}/{s3_bucket}/{sub_path}/{output_name}"
                ...
                item_dict = {
                    ...
                    "geometry": None,
                    "bbox": None,
                    "properties": {"datetime": now_str},
                    "links": [
                        {"rel": "self", "href": f"{base_url}/item.json"},
                        {"rel": "root", "href": f"{base_url}/catalog.json"},
                        {"rel": "collection", "href": f"{base_url}/collection.json"},
                    ],
                    "assets": {
                        "data": {
                            "href": file_http_uri,
                ...
                collection_dict = {
                    ...
                    "id": output_name,
                    "stac_version": "1.0.0",
                    "description": f"Collection for output {output_name}",
                    "links": [
                        {"rel": "self", "href": f"{base_url}/collection.json"},
                        {"rel": "root", "href": f"{base_url}/catalog.json"},
                        {"rel": "item", "href": f"{base_url}/item.json"},
                    ],
                    "title": output_name,
                    "extent": {
                        "spatial": {"bbox": [[-180, -90, 180, 90]]},
                        "temporal": {"interval": [[now_str, None]]},
                    },
                    "license": "proprietary",
                }
                ...
                catalog_dict = {
                    "type": "Catalog",
                    "id": f"{sub_path}-{output_name}",
                    "stac_version": "1.0.0",
                    "description": f"Catalog for output {output_name}",
                    "links": [
                        {"rel": "self", "href": f"{base_url}/catalog.json"},
                        {"rel": "root", "href": f"{base_url}/catalog.json"},
                        {"rel": "child", "href": f"{base_url}/collection.json"},
                    ],
                }

                for key, data in [
                    (f"{sub_path}/{output_name}/item.json", item_dict),
                    (f"{sub_path}/{output_name}/collection.json", collection_dict),
                    (f"{sub_path}/{output_name}/catalog.json", catalog_dict),
                ]:
                ...
                catalog_http_uri = f"{base_url}/catalog.json"
                logger.info(f"Created STAC catalog at {catalog_http_uri}")
                updated_output[output_name] = {"value": catalog_http_uri}

            except Exception as e:
                logger.error(f"Failed to stage out File output '{output_name}': {e}")
                logger.exception(e)
